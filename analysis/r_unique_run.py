...
pu.plot_graphs([G, sze, fsze, fG ], [f"{dset_name} G {refinement}", f"sze {k}", "filtered sze", "filtered G"])#, FILENAME=f"./data_unique_run/plots/{dset_name}.eps", save=True)
...
for i in np.arange(0,1,0.05):
    ...
    print("{:.2f} : {:.4f} : {:.4f} : {:.4f}".format(i, pd.density(usze, weighted=False), density, np.linalg.norm(G-usze)/G.shape[0]))
